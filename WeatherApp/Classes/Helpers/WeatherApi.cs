...
namespace WeatherApp.Classes.Helpers
{
    public class WeatherApi
    {
        ...
        public async Task<Root> GetWeatherForecastAsync(string city, Panel loading)
        {
            try
            {
                ...
                var url = _baseUrl + "forecast.json?key=" + _apiKey + "&q=" + city + "&days=6";
                using (var client = new HttpClient())
                {
                    ...
                    Root root = JsonExtensions.ToObject<Root>(json);
            ...
            catch (HttpRequestException ex)
            {
                ...
                MessageBox.Show($"HTTP isteği başarısız oldu: {ex.Message}", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                throw;
            }
            finally
            {
